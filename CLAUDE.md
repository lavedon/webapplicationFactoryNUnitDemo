...
6. `PesterTests/` holds a black-box Pester 5 twin of the NUnit suite: real HTTP requests against a RUNNING API (default `https://localhost:7016`, override with `TACOBELL_API_URL`), real JWT from `/token` (no auth bypass). Mutation tests create their own rows since the server DB persists; run manually with `Invoke-Pester -Path ./PesterTests`.

7. CLI commands: `runner pester` (run the Pester suite against a live API, `-u|--url` to override the base URL, probes reachability first); `runner run` (whole suite, Spectre table/tree of pass/fail/duration + failure details); `runner run --filter <expr>` / `-f` (pass-through to `dotnet test --filter`); `runner interactive` (menu mode: root menu picks NUnit or Pester, second menu has "Run ALL tests" first then every individual test; the equivalent manual command (dotnet test --filter / Invoke-Pester -FullNameFilter) is always printed before running). Bare `runner` with no args shows a Spectre help screen (commands, switches, examples) rather than running anything.
